...
from tests.factories import ScheduleFactory, ClassFactory
...
class TestScheduleFilters:
    ...
    def test_filter_by_class_name(self, api_client):
        ...
        assert all(item['class_group']['name'] == '9A' for item in response.data['results'])
    ...
    def test_filter_for_today_weekday(self, mock_get_current_day, api_client):
        ...
        assert response.status_code == status.HTTP_200_OK
        assert len(response.data['results']) == 2
        assert all(item['day_of_week'] == 'Wednesday' for item in response.data['results'])
