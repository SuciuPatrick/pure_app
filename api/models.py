...
class Schedule(models.Model):
    ...
    day_of_week = models.IntegerField(
        choices=DAYS_OF_WEEK, validators=[MinValueValidator(1), MaxValueValidator(5)]
    )
    ...
    class Meta:
        ...
        unique_together = [
            [
                ...
                "day_of_week",
                "hour",
            ],  # Prevent multiple lessons at the same time for a class
            [
                "subject",
                "day_of_week",
                "hour",
            ],  # Prevent teacher from teaching multiple classes at the same time
        ]
...
def update_old_class_student_count(sender, instance, **kwargs):
    if instance.pk:  # Only for existing students
        try:
            ...
            if (
                old_student.class_group
                and old_student.class_group != instance.class_group
            ):
                # Update the old class count
                old_student.class_group.student_count = (
                    old_student.class_group.students.count() - 1
                )
